...
class otimizacao:
    ...
    def valida_acao_teste_objetivo(self, p, m, lucro_anterior, lucro):
        if (p >= 0 and m >= 0):
            if(p <= 40 and m <= 30):
                if((20 * p + 30 * m) <= 1200):
                    if(lucro > lucro_anterior):
                        return True
        return False
        ...
        if (self.p >= 0 and self.m >= 0):
            if(self.p <= 40 and self.m <= 30):
                if((20 * self.p + 30 * self.m) <= 1200):
                    if(self.lucro > self.lucro_anterior):
                        return True
        return False
    ...
    def valida_acao(self, acao):
        p = self.p
        m = self.m
        lucro_anterior = self.lucro
        lucro = 0
        if(acao == ("p++", "m++")):
            p += 1
            m += 1
            lucro += 1000 * p + 1800 * m
            return self.valida_acao_teste_objetivo(p, m, lucro_anterior, lucro)
        if(acao == ("p++", "m--")):
            p += 1
            m -= 1
            lucro += 1000 * p + 1800 * m
...
def hill_climbing():
    ...
    while True:
        current[0].acao()
        current[0].geraAcoes()
        if(not current[0].teste_objetivo() or len(current[0].acoes_validas) == 0):
            return current


def main():
    estado_final = hill_climbing()
    print("Utilizando Hill Climbing:")
    print ("Estado Final:")
