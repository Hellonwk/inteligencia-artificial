...
def busca_custo_uniforme():
    print("Usando Busca de Custo Uniforme: ")
    inicio = Torres.torres()
    print("")
    inicio.toString()
    print("")
    estados_abertos = [inicio]
    custo = -1
    fechado = list()
    while estados_abertos:
        X = estados_abertos[0].acaoProximoConsiderandoG(custo, estados_abertos)
        custo += 1
        if X.ehEstadoFinal():
            return X
        else:
...
def main():
    estado_final = busca_custo_uniforme()
    print ("Estado Final:")
